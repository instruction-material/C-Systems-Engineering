...
static bool validate_row(unsigned int sample_id, int temp_milli_c,
                         unsigned int voltage_mv, unsigned int flags) {
    return sample_id <= 255u && temp_milli_c >= -40000 &&
           temp_milli_c <= 125000 && voltage_mv <= 5000u && flags <= 255u;
}
...
int main(int argc, char** argv) {
    ...
    while (fgets(line, sizeof(line), input) != NULL) {
        ...
        fprintf(output, "%u,%d,%d,%u,0x%02X,%s\n", sample_id, temp_milli_c,
                temp_milli_f, voltage_mv, flags, status);
        transformed_rows += 1;
    }
    ...
    printf("math note         : temperature stays in fixed-point milli-units "
